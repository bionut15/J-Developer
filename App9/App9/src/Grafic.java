public class Grafic {
    ...
    static double s(double f, double fd1min, double fd1max) {
        return (f - fd1min) / (fd1max - fd1min);
    }   
    static double t(double f, double fd2min, double fd2max) {
        return (f - fd2min) / (fd2max - fd2min);
    }
    ...
    public static void main(String[] args) {
        ...
        for (i = 0; i < x.length; ++i) {
            ...
            int posyd2 = (int) (LY * s(yd1[i], ymin, ymax));
            String s = "";
            for (int k = 0; k <= LY; ++k) {
                    if (k == posyd2) {
                        s += "d";
